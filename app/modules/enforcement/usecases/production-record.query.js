...
export class ProductionRecordQueryService {
  list = async (req, queryParams = {}) => {
    ...
    if (queryParams.facilityId) {
      where.facilityId = queryParams.facilityId;
    }

    const { count, rows } = await models.ProductionRecord.findAndCountAll({
      where,
  ...
  getById = async (req, id) => {
    ...
    return models.ProductionRecord.findOne({
      where,
      include: [
        { association: 'organization', required: false },
        { association: 'facility', required: false },
